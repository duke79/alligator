...
class MySQL():
    def __init__(self):
        ...
        # Queries
        self.QUERY_CREATE_USER = "create user '%s'@'%s' identified by '%s';" \
                                 % (self.config["user"], self.config["host"], self.config["password"])
        self.QUERY_GRANT_PERMISSION = "grant all privileges on %s.* to '%s'@'%s' with grant option;" \
                                      % (self.config["db"], self.config["user"], self.config["host"])
        self.QUERY_CREATE_DB = "CREATE DATABASE `%s`;" % (self.config["db"])
        self.QUERY_CHANGE_DB = "use %s;" % (self.config["db"])
        ...
        self.QUERY_SELECT_ALL_TABLES = """select table_name from information_schema.tables where table_type="BASE TABLE" and table_schema="%s";""" % (
        self.config["db"])
